  ...
  Future<void> getLiveMatchList(context) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        notifyListeners();
        if (data['status'] == true) {
          List liveData = data["data"];
          liveDataListLength = data["data"];
          liveDataList.addAll(dateWiseListResponse1(liveData));
...
List homeOpenAppAds = [];
  ...
  Future<void> sendDeviceIdToServer(context, String device_id) async {
    isLoadingPlayer = true;
    var request = http.MultipartRequest(
        'POST', Uri.parse(Constant.fcmDevice));
    request.fields.addAll({"device_id": device_id});
    try {
      http.StreamedResponse response = await request.send();
      if (kDebugMode) {
        print("response  ${response.statusCode}");
